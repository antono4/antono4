...
function generateCity3D({ weeks, totalContributions, firstDay, lastDay }) {
  ...
  weeks.forEach(week => {
    week.contributionDays.forEach(day => {
      allContribs.push(day.contributionCount);
    });
  });
  ...
  weeks.forEach((week, weekIndex) => {
    week.contributionDays.forEach((day, dayIndex) => {
      const x = startX + (weekIndex * (blockWidth + gap));
      const y = startY + (dayIndex * (blockDepth + gap));
...
generate3DSVG().catch(console.error);
